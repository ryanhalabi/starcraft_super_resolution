...
class ModelTrainer:
    ...
    def __init__(self, model, save=False):
        ...
        if os.path.isdir( env.output_data / 'logs' / self.model.name):
          shutil.rmtree( env.output_data / 'logs' / 'fit' / self.model.name )
        os.mkdir( env.output_data / 'logs' / 'fit' / self.model.name )
    ...
    def train(self, images, epochs, save):
        ...
        for i in range(epochs):
          os.mkdir( env.path / 'logs' / 'fit' / self.model.name / str(i) )

          tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir= str(env.path / 'logs' / 'fit' / self.model.name / str(i)), histogram_freq=1)

          self.model.model.fit(self.X, self.Y, epochs = save, verbose=1, callbacks=[tensorboard_callback]) 
          # self.model.model.fit(self.X, self.Y, epochs = save, verbose=1) 
          print(f"Epoch {i} Loss: {self.model.model.history.history['loss'][-1]}")
          self.predict(images, i+1)
          if self.save:
              self.model.save_model(f"{self.model.name}_{i+1}.hdf5")
    ...
    def save_initial(self, images):
        ...
        padding = int((self.model.conv_size-1)/2)
        up_samples = np.array([x[padding:-padding, padding:-padding,:] for x in up_samples])

        self.log_images(Y, 0)
        self.log_images(up_samples, 0)

    def log_images(self, images, step):
        file_writer = tf.summary.create_file_writer(str(env.path / 'logs' / 'fit' / self.model.name ))
